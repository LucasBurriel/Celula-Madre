...
Executed in subprocess with timeout for safety.
"""
import subprocess
import os
...
TOOL_DEFINITIONS = [
    {
        ...
        "function": {
            "name": "bash",
            "description": "Run a bash command. State is NOT persistent between calls. Use for running tests, checking files, installing packages, etc.",
            "parameters": {
                "type": "object",
                "properties": {
                    "command": {
    ...
    },
    {
        ...
        "function": {
            "name": "editor",
            "description": "View, create, or edit files. Use 'view' to see file contents or directory listing, 'create' for new files, 'edit' to overwrite existing files.",
            "parameters": {
                "type": "object",
                "properties": {
                    "command": {
...
def _run_editor(command, path, file_text=None, workdir=None):
    """File editor tool."""
    try:
        ...
        if command == "view":
            ...
            if not path_obj.exists():
                # Be lenient - create if doesn't exist
                path_obj.parent.mkdir(parents=True, exist_ok=True)
            path_obj.write_text(file_text or "")
